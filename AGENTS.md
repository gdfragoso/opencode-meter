...
| Cost per result | `src/api/services/cost.ts` | Whole-window only. Denominators from session_files (action <> 'read'); ratios are null, never 0. Splitting by agent or tool put cost and result on different sessions under delegation — removed, not fixed. |
