...
export function CostPerToolTable({ data }: { data: CostEfficiencyResponse | null }) {
  if (!data) return <LoadingPlaceholder rows={4} />;
  if (data.byTool.length === 0) return <EmptyState />;
      ...
      <table className="w-full text-xs">
        <thead>
          <tr className="text-cyber-cyan/40 uppercase tracking-[0.12em] border-b border-cyber-cyan/10">
            ...
            <th className="text-right py-2 pr-4 font-normal">Calls</th>
            <th className="text-right py-2 pr-4 font-normal">Cost</th>
            <th className="text-right py-2 pr-4 font-normal">Files</th>
            <th className="text-right py-2 pr-4 font-normal">Per Call</th>
            <th className="text-right py-2 font-normal">Per File</th>
          </tr>
        </thead>
        <tbody>
          {data.byTool.map((t) => (
            <tr key={t.tool} className="border-b border-cyber-cyan/5 hover:bg-cyber-cyan/5 transition-colors">
              <td className="py-2 pr-4 text-cyber-cyan truncate max-w-[200px]">{t.tool}</td>
              <td className="py-2 pr-4 text-right tabular-nums text-cyber-cyan/70">{fmtNum(t.calls)}</td>
              <td className="py-2 pr-4 text-right tabular-nums">{fmtUSD(t.cost)}</td>
              <td className="py-2 pr-4 text-right tabular-nums text-cyber-cyan/70">{fmtNum(t.files)}</td>
              <td className="py-2 pr-4 text-right tabular-nums text-cyber-cyan/70">
                {fmtUSD(t.costPerCall)}
              </td>
              <td
                className="py-2 text-right tabular-nums"
                title={ratioTitle(t.costPerFile, "file change")}
              >
                {fmtUSD(t.costPerFile)}
              </td>
            </tr>
          ))}
        </tbody>
